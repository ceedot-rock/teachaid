/**
 * TEACHAiD onboarding — adult further education
...
(function () {
  ...
  function renderInterestPicker() {
    ...
    home.innerHTML =
      ...
      "<h3>What do you want to study next?</h3>" +
      "<p>You're an adult learner. Pick one or more areas of further education — we'll suggest real college-style and professional classes, not kids' drills.</p>" +
      "</div>" +
  ...
  function renderRecommendations(ids) {
    ...
    home.innerHTML =
      ...
      "<p>Each class has a dedicated teacher. When you open one, you'll start with a real conversation — getting to know each other — before the formal lessons.</p>" +
      "</div>" +
      (cards ||
        "<p class=\"label\">No matches yet — change interests.</p>") +
    ...
    // Fill full catalog if renderBuiltinCatalog exists
    if (typeof window.renderBuiltinCatalog === "function") {
      var hold = document.getElementById("homeCatalog");
      if (!hold) {
        var d = document.createElement("div");
        d.id = "homeCatalog";
        var slot = document.getElementById("homeCatalogOnboard");
      ...
      try {
        ...
        if (src && dest && src !== dest) dest.innerHTML = src.innerHTML;
      } catch (e) {}
    }
  }

  function personableOpen(bookId) {
    if (typeof window.openBook !== "function") return;
    window.openBook(bookId);
    if (!window.BOOKS || !window.BOOKS[bookId]) return;
    if (hasMet(bookId)) return;

    // Replace stiff welcome with a meet-and-greet prompt
    try {
      var log = document.getElementById("chatLog");
      if (log) log.innerHTML = "";
      var b = window.BOOKS[bookId];
      var intro =
        "Hey — I'm " +
        b.teacher +
        ". Before we dig into “" +
      ...
      if (typeof window.addBubble === "function") {
        window.addBubble("bot", intro, false);
      }
      if (window.histories && typeof window.getHist === "function") {
      ...
      if (input) {
        input.placeholder = "Tell " + b.teacher + " a bit about yourself…";
      }
    } catch (e) {}
  }

  // Public: wrap openBook once DOM + main app ready
  function install() {
    if (!window.openBook || window.__teachaidOpenWrapped) {
      // still try interest UI
      if (document.getElementById("home")) {
        var path = location.hash || "";
        if (!path || path === "#" || path === "#home") renderInterestPicker();
      }
      return;
    }
    ...
    // Avoid double-open: personableOpen calls orig already path
    // Fix: personableOpen shouldn't call openBook again
  }

  // Fixed install without recursion
  ...
  function bootHome() {
    ...
    // Only override default home when showing home tab
    renderInterestPicker();
  }
