...
(function (global) {
  ...
  function dropCounting() {
    ["s1", "s2", "s3"].forEach(function (id) {
      if (global.BUILTIN_BOOKS) delete global.BUILTIN_BOOKS[id];
      if (global.BOOKS) delete global.BOOKS[id];
    });
  ...
  function renderRecs(ids) {
    ...
    if (!home) return;
    dropCounting();
    var books = global.BOOKS || global.BUILTIN_BOOKS || {};
  ...
  function wrapOpenBook() {
    ...
    global.openBook = function (id) {
      ...
      try {
        ...
        var b = (global.BOOKS || global.BUILTIN_BOOKS || {})[id];
        if (!b) return;
        var intro =
          "Hey — I'm " + b.teacher +
